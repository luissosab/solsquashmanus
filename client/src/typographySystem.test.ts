...
const magneticButton = readFileSync(
  new URL("./components/MagneticButton.tsx", import.meta.url),
  "utf8"
);
const structuralBlocks = readFileSync(
  new URL("./components/StructuralContentBlocks.tsx", import.meta.url),
  "utf8"
);
const home = readFileSync(new URL("./pages/Home.tsx", import.meta.url), "utf8");
const newToSquash = readFileSync(
  new URL("./pages/NewToSquash.tsx", import.meta.url),
  "utf8"
);
const ourStory = readFileSync(
  new URL("./pages/OurStory.tsx", import.meta.url),
  "utf8"
);
const playAndPricing = readFileSync(
  new URL("./pages/PlayAndPricing.tsx", import.meta.url),
  "utf8"
);
const pageShell = readFileSync(
  new URL("./components/PageShell.tsx", import.meta.url),
  "utf8"
);
...
describe("Sol typography grammar", () => {
  ...
  it("uses only local public media and font files at runtime", () => {
    ...
    expect(runtimeSource).not.toMatch(
      /https?:\/\/[^\s"')]+\.(?:gif|jpe?g|mp4|otf|png|svg|ttf|webm|webp|woff2?)/i
    );

    const localAssets = Array.from(
      runtimeSource.matchAll(/["'(](\/(?:fonts|media)\/[^"')\s]+)/g),
      match => match[1]
    );
  ...
  it("keeps heading emphasis in the display system instead of arbitrary italics", () => {
    expect(css).toMatch(
      /\.site-frame main h1 em,[\s\S]*?font-family:var\(--font-display\);[\s\S]*?font-style:normal;/
    );
  });

  it("uses the sitewide display and body grammar for Club and footer copy", () => {
    expect(css).toMatch(
      /\.club-intro--lifestyle \.club-intro__section-intro h2[\s\S]*?font-family:var\(--font-display\);[\s\S]*?font-style:normal;/
    );
    expect(css).toMatch(
      /\.club-intro--lifestyle \.club-intro__section-intro \.club-intro__promise[\s\S]*?font-family:var\(--font-body\);[\s\S]*?font-style:normal;/
    );
    expect(css).toMatch(
      /\.club-footer__main \.club-footer__promise[\s\S]*?font-family:var\(--font-body\);[\s\S]*?font-style:normal;/
    );
  });

  it("uses the body family and regular style for supporting copy", () => {
    expect(css).toMatch(
      /\.home-path-card p,[\s\S]*?\.pricing-card > p[\s\S]*?font-family:var\(--font-body\);[\s\S]*?font-style:normal;/
    );
  });
  ...
  it("uses selective mural accents and distinct Values card colors", () => {
    ...
    expect(css).toMatch(
      /\.values-list article:nth-child\(1\)[\s\S]*?\.values-list article:nth-child\(4\)/
    );
    expect(css).toContain(
      ".values-list article > span { color:inherit !important;"
    );
  });
  ...
  it("uses matching dots on every decorative ball and rejects generic L/U court marks", () => {
    ...
    const renderedVariants = Array.from(
      renderedMarkup.matchAll(/squash-ball--([a-z-]+)/g),
      match => match[1]
    ).sort();

    expect(Array.from(new Set(renderedVariants))).toEqual([
      "beginner",
      "faq",
      "hero",
      "home-club",
    ...
    expect(css).toMatch(
      /\.squash-ball--offer i,[\s\S]*?background:var\(--sol-navy\)/
    );
    expect(css).toMatch(
      /\.squash-ball--beginner i,[\s\S]*?background:var\(--sol-teal\)/
    );
    expect(css).toMatch(
      /\.squash-ball--values i,[\s\S]*?background:var\(--sol-navy\)/
    );
  ...
  it("keeps PERFECT emphasized and combines packages with the price disclosure", () => {
    expect(newToSquash).toMatch(
      /<motion\.em[\s\S]*?>\s*PERFECT\.\s*<\/motion\.em>/
    );
    expect(css).toMatch(
      /\.entry-hero__copy h1 em[\s\S]*?font-style:italic !important/
    );
  ...
  it("separates adjacent Play color rooms and uses a lighter green Home overlay", () => {
    expect(structuralBlocks).toContain(
      '<WaveDivider source="green" destination="navy" />'
    );
    expect(playAndPricing).toContain(
      '<WaveDivider source="navy" destination="navy" />'
    );
    expect(css).toContain(
      ".pricing-section--merged-offers { background-color:var(--sol-navy) !important;"
    );
  ...
  it("uses one repeated text-color pattern per Values and Team card group", () => {
    expect(css).toMatch(
      /\.values-list article > span \{[\s\S]*?color:var\(--sol-mustard\) !important;/
    );
    expect(css).toMatch(
      /\.values-list article h3,[\s\S]*?\.values-list article p \{[\s\S]*?color:var\(--sol-light\) !important;/
    );
    expect(css).toMatch(
      /\.profile-card__copy > span,[\s\S]*?\.profile-card__copy > button \{[\s\S]*?color:var\(--sol-mustard\) !important;/
    );
    expect(css).toMatch(
      /\.profile-card__copy > h3,[\s\S]*?\.profile-card__copy > p:not\(\.sr-only\) \{[\s\S]*?color:var\(--sol-light\) !important;/
    );
  });

  it("adds one subtle shadow to every squash-ball variant", () => {
    expect(css).toMatch(
      /\.squash-ball \{[\s\S]*?filter:drop-shadow\(\.42rem \.58rem \.38rem rgba\(12,37,74,\.5\)\);/
    );
  });

  it("uses repeated text-role patterns in the first-timer and session card groups", () => {
    expect(newToSquash).not.toContain(
      'offer.tone === "mango" ? "button--navy" : "button--mango"'
    );
    expect(newToSquash).toContain('className="button--mango"');
    expect(css).toMatch(
      /\.first-step-card__top,[\s\S]*?\.first-step-card \.booking-handoff-note \{[\s\S]*?color:var\(--sol-mustard\) !important;/
    );
    expect(css).toMatch(
      /\.session-card__time small,[\s\S]*?\.session-card__info p \{[\s\S]*?color:var\(--sol-mustard\) !important;/
    );
    expect(css).toMatch(
      /\.session-card__time span,[\s\S]*?\.session-card__info h3 \{[\s\S]*?color:var\(--sol-light\) !important;/
    );
  });
