...
export default function PlayAndPricing() {
  ...
  return (
    <>
      <section className="play-hero play-hero--mural"><div><p className="eyebrow eyebrow--cyan">WAYS TO PLAY</p><h1>PICK YOUR<br />WAY <em>IN.</em></h1><p>Court bookings, open play, coached sessions and lessons. With or without a membership.</p></div><figure className="play-hero__mural"><img src="/manus-storage/sol-mural-feature_9dd29c1b.png" alt="Sol Squash mural artwork" /><span className="squash-ball squash-ball--play" aria-hidden="true"><i /><i /></span></figure></section>
      <WaveDivider destination="green" />
      <section className="ways-section ways-section--real"><div className="section-rail"><span>WAYS TO PLAY</span><i /></div><div className="ways-list">{waysToPlay.map((way, index) => { const isActive = activeWay === index; return <motion.button layout key={way.name} className={`way-card ${isActive ? "way-card--active" : ""}`} onClick={() => setActiveWay(isActive ? -1 : index)} aria-expanded={isActive}><div className="way-card__top"><span>0{index + 1}</span><span>{isActive ? <X size={21} /> : <Plus size={21} />}</span></div><h2>{way.name}</h2><p>{way.short}</p>{isActive && <motion.div className="way-card__expanded" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}><span>{way.detail}</span><p>{way.long}</p></motion.div>}</motion.button>; })}</div></section>
      <WaveDivider destination="navy" />
      <section className="pricing-section pricing-section--real"><div className="pricing-heading"><p className="eyebrow eyebrow--cyan">ADULT PRICING</p><h2>YOUR GAME.<br />YOUR <em>PACE.</em></h2></div><div className="pricing-switch" role="group" aria-label="Pricing tier"><button onClick={() => setTier("guest")} className={tier === "guest" ? "is-selected" : ""}>DROP-IN (GUEST)</button><button onClick={() => setTier("member")} className={tier === "member" ? "is-selected" : ""}>SOL CLUB ($239/MO)</button></div><div className="pricing-card"> <div className="pricing-card__head"><span>{tier === "guest" ? "DROP-IN (GUEST)" : "SOL CLUB ($239/MO)"}</span><strong>{tier === "guest" ? "PLAY WHEN YOU WANT" : "MAKE SOL YOUR HOME"}</strong></div>{comparisons.map((row) => <div className="price-row" key={row.label}><span>{row.label}{row.tip && <Tooltip><TooltipTrigger asChild><button className="info-button" aria-label={`More information about ${row.label}`}><Info size={14} /></button></TooltipTrigger><TooltipContent>{row.tip}</TooltipContent></Tooltip>}</span><b>{tier === "guest" ? row.guest : row.member}</b></div>)}<div className="pricing-card__note"><Tooltip><TooltipTrigger asChild><button>MEMBERSHIP NOTES <Info size={15} /></button></TooltipTrigger><TooltipContent className="membership-tooltip">7-day booking window. 1 guest pass every month. Racket rental on us.</TooltipContent></Tooltip></div><MagneticButton href="#" onClick={() => setCheckoutOpen(true)} className="button--mango">BOOK A SESSION</MagneticButton></div></section>
      <WaveDivider destination="mango" />
      <section className="junior-section junior-section--real"><div className="junior-section__painted-label" aria-hidden="true"><span className="squash-ball squash-ball--junior"><i /><i /></span></div><div><p className="eyebrow">FOR THE NEXT GENERATION</p><h2>SOL JUNIOR<br /><em>($179/MO)</em></h2><p>Ages 7 to 16. A membership for juniors. 7 group sessions included per month. Extra session $40. Drop In: $50.</p></div><figure className="junior-section__image"><img src="/manus-storage/sol-facility-blue-court_a3f6989d.png" alt="The blue squash court at Sol" /></figure></section>
      <WaveDivider destination="green" />
      <section className="faq-guide" id="sol-guide"><div className="faq-guide__intro"><p className="eyebrow eyebrow--cyan">EVERYTHING YOU NEED TO KNOW</p><h2>THE <em>SOL</em><br />GUIDE.</h2><p>Start with the question on your mind. Every detail is here, without the information overload.</p></div><div className="faq-topic-nav" role="tablist" aria-label="Sol guide topics">{faqTopics.map((topic, index) => <button key={topic.id} role="tab" aria-selected={activeTopic === topic.id} onClick={() => setActiveTopic(topic.id)} className={activeTopic === topic.id ? "is-active" : ""}><span>0{index + 1}</span>{topic.label}</button>)}</div><div className="faq-topic-panels">{faqTopics.map((topic) => <section key={topic.id} role="tabpanel" hidden={activeTopic !== topic.id} className="faq-topic-panel"><div className="faq-topic-panel__head"><span className="squash-ball squash-ball--faq" aria-hidden="true"><i /><i /></span><div><p>{topic.cue}</p><h3>{topic.label}</h3></div><b>{topic.questions.length} QUESTIONS</b></div><Accordion type="single" collapsible className="faq-list">{topic.questions.map(([question, answer], index) => <AccordionItem key={question} value={`${topic.id}-${index}`}><AccordionTrigger>{question}</AccordionTrigger><AccordionContent>{answer}</AccordionContent></AccordionItem>)}</Accordion></section>)}</div></section>
      <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}><DialogContent className="checkout-dialog"><DialogTitle>ONE MORE GOOD IDEA.</DialogTitle><span className="squash-ball squash-ball--dialog" aria-hidden="true"><i /><i /></span><p>Add a 5-pack for $250 and save $25.</p><button className="dialog-close" onClick={() => setCheckoutOpen(false)}>NOT TODAY</button></DialogContent></Dialog>
    </>
  );
}
