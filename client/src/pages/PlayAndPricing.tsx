...
import { JoinContinuationBlocks, TasteOfSolBlock } from "@/components/StructuralContentBlocks";
import data from "@/lib/mock-playbypoint.json";
import { PLAYBYPOINT_BOOKING_URL, PLAYBYPOINT_COURT_URL, PLAYBYPOINT_OPEN_PLAY_URL, PLAYBYPOINT_PRIVATE_LESSON_URL, PLAYBYPOINT_SESSIONS_URL, PLAYBYPOINT_SOL_CLUB_URL, PLAYBYPOINT_SOL_JUNIOR_MEMBERSHIP_URL } from "@/lib/booking";
import { Link } from "wouter";

const waysToPlay = [
  { name: "Book a Court", short: "Your court, your hour.", long: "Your court, your hour. Play with a friend, another Sol player or rotate with a group of three.", detail: "MAKE A RALLY YOUR OWN", action: "BOOK A COURT", href: PLAYBYPOINT_COURT_URL },
  { name: "Open Play", short: "Our social squash mixer.", long: "Our social squash mixer. Three hours of games, court rotations and players matched by level.", detail: "MEET YOUR NEXT RALLY PARTNER", action: "FIND OPEN PLAY", href: PLAYBYPOINT_OPEN_PLAY_URL },
  { name: "Coached Sessions", short: "Train in a small group with Bruna or Vini.", long: "Train in a small group with Bruna or Vini — Drills & Matches, Clinics and Women Play.", detail: "LEARN, PLAY, REPEAT", action: "SEE COACHED SESSIONS", href: PLAYBYPOINT_SESSIONS_URL },
  { name: "Private Lessons", short: "Your game. Our full attention.", long: "Your game. Our full attention. One-on-one with Bruna or Vini.", detail: "ONE-ON-ONE ON COURT", action: "BOOK A PRIVATE LESSON", href: PLAYBYPOINT_PRIVATE_LESSON_URL },
];

const comparisons = [
  { label: "Court Access", guest: "$120/court · up to 3", member: "Included (1 hr/day).", tip: "Solo booking $60. Extra member hours $20 a player." },
...
const faqTopics = [
  ...
  { id: "ways", label: "HOW IT WORKS", cue: "HOW YOU CAN GET ON COURT", questions: [
...
const faqTopicActions: Record<string, { href: string; label: string; external?: boolean }> = {
  ...
  booking: { href: PLAYBYPOINT_BOOKING_URL, label: "BOOK A SESSION", external: true },
};

function renderAnswer(answer: string) {
  return answer.split(/(https:\/\/[^\s]+)/g).map((part, index) => part.startsWith("https://") ? <a key={`${part}-${index}`} href={part}>{part}</a> : part);
}
...
export default function PlayAndPricing({ section = "all" }: { section?: ContentSection }) {
  ...
  return (
    <>
      {showPlay && <><section className="play-hero play-hero--mural"><div><p className="eyebrow eyebrow--cyan">WAYS TO PLAY</p><h1>PICK YOUR<br />WAY <em>IN.</em></h1><p>Court bookings, open play, coached sessions and lessons. With or without a membership.</p></div><figure className="play-hero__mural"><img src="/media/sol-facility-blue-court_a3f6989d.png" alt="The blue squash court at Sol" /></figure></section><WaveDivider source="navy" destination="green" /><section className="ways-section ways-section--real"><div className="section-rail"><span>WAYS TO PLAY</span><i /></div><div className="ways-list">{waysToPlay.map((way, index) => { const isActive = activeWay === index; return <motion.article layout key={way.name} className={`way-card ${isActive ? "way-card--active" : ""}`}><button type="button" className="way-card__toggle" onClick={() => setActiveWay(isActive ? -1 : index)} aria-expanded={isActive} aria-controls={`play-option-${index}`}><div className="way-card__top"><span>0{index + 1}</span><span>{isActive ? <X size={21} /> : <Plus size={21} />}</span></div><h2>{way.name}</h2><p>{way.short}</p></button>{isActive && <motion.div id={`play-option-${index}`} className="way-card__expanded" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}><span>{way.detail}</span><p>{way.long}</p><MagneticButton href={way.href} className="button--mango">{way.action}</MagneticButton><p className="booking-handoff-note">Choose a live time and complete your booking next.</p></motion.div>}</motion.article>; })}</div></section><TasteOfSolBlock /></>}
      {showJoin && <>{showPlay && <WaveDivider source="navy" destination="navy" />}<section className="pricing-section pricing-section--real"><div className="pricing-heading"><p className="eyebrow eyebrow--cyan">ADULT PRICING</p><h2>ADULT<br /><em>PRICING.</em></h2></div><div className="pricing-switch" role="group" aria-label="Pricing tier"><button onClick={() => setTier("guest")} className={tier === "guest" ? "is-selected" : ""}>DROP-IN · GUEST</button><button onClick={() => setTier("member")} className={tier === "member" ? "is-selected" : ""}>SOL CLUB · $239/MO</button></div><div className="pricing-card"> <div className="pricing-card__head"><span>{tier === "guest" ? "DROP-IN · GUEST" : "SOL CLUB · $239/MO"}</span><strong>{tier === "guest" ? "DROP-IN · GUEST" : "SOL CLUB · $239/MO"}</strong></div>{comparisons.map((row) => <div className="price-row" key={row.label}><span>{row.label}{row.tip && <Tooltip><TooltipTrigger asChild><button className="info-button" aria-label={`More information about ${row.label}`}><Info size={14} /></button></TooltipTrigger><TooltipContent className="membership-tooltip">7-day booking window. 1 guest pass every month. Racket rental on us.</TooltipContent></Tooltip>}</span><b>{tier === "guest" ? row.guest : row.member}</b></div>)}<div className="pricing-card__note"><Tooltip><TooltipTrigger asChild><button>MEMBERSHIP NOTES <Info size={15} /></button></TooltipTrigger><TooltipContent className="membership-tooltip">7-day booking window. 1 guest pass every month. Racket rental on us.</TooltipContent></Tooltip></div><MagneticButton href={PLAYBYPOINT_SOL_CLUB_URL} className="button--mango">JOIN SOL CLUB</MagneticButton><p className="booking-handoff-note">Choose your membership and complete it next.</p></div></section><WaveDivider source="navy" destination="mango" /><section className="junior-section junior-section--real"><div className="junior-section__painted-label" aria-hidden="true"><img src="/media/sol-sun-repaired-navy_6dbcb19f.png" alt="" /></div><div><p className="eyebrow">FOR THE NEXT GENERATION</p><h2>SOL JUNIOR<br /><em>$179/MO</em></h2><p>Ages 7 to 16. A membership for juniors. 7 group sessions included per month. Extra session $40. Drop In: $50.</p><MagneticButton href={PLAYBYPOINT_SOL_JUNIOR_MEMBERSHIP_URL} className="button--navy">JOIN SOL JUNIOR</MagneticButton><p className="booking-handoff-note">Choose a live time and complete your booking next.</p></div></section><JoinContinuationBlocks /></>}
      {showFaq && <>{showJoin && <WaveDivider source="mango" destination="navy" />}<section className="faq-guide" id="sol-guide"><img className="mural-sun mural-sun--faq" src="/media/sol-sun-repaired-orange_e644a385.png" alt="" aria-hidden="true" /><div className="faq-guide__intro"><p className="eyebrow eyebrow--cyan">EVERYTHING YOU NEED TO KNOW.</p><h2>EVERYTHING<br /><em>YOU NEED</em><br />TO KNOW.</h2></div><div className="faq-topic-nav" role="tablist" aria-label="Sol guide topics">{faqTopics.map((topic, index) => <button key={topic.id} role="tab" aria-selected={activeTopic === topic.id} onClick={() => setActiveTopic(topic.id)} className={activeTopic === topic.id ? "is-active" : ""}><span>0{index + 1}</span>{topic.label}</button>)}</div><div className="faq-topic-panels">{faqTopics.map((topic) => { const action = faqTopicActions[topic.id]; const actionContent = <>{action.label} <span aria-hidden="true">↗</span></>; return <section key={topic.id} role="tabpanel" hidden={activeTopic !== topic.id} className="faq-topic-panel"><div className="faq-topic-panel__head"><span className="squash-ball squash-ball--faq" aria-hidden="true"><i /><i /></span><div><h3>{topic.label}</h3></div><b>{topic.questions.length} QUESTIONS</b></div><Accordion type="single" collapsible className="faq-list">{topic.questions.map(([question, answer], index) => <AccordionItem key={question} value={`${topic.id}-${index}`}><AccordionTrigger>{question}</AccordionTrigger><AccordionContent>{renderAnswer(answer)}</AccordionContent></AccordionItem>)}</Accordion><div className="faq-topic-panel__action">{action.external ? <a href={action.href} className="text-link">{actionContent}</a> : <Link href={action.href} className="text-link">{actionContent}</Link>}</div></section>; })}</div></section></>}
      {showPlay && !showJoin && <WaveDivider source="navy" destination="navy" />}
      {showJoin && !showFaq && <WaveDivider source="green" destination="navy" />}
