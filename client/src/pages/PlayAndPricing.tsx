...
export default function PlayAndPricing() {
  ...
  return (
    <>
      ...
      <section className="pricing-section pricing-section--real"><div className="pricing-heading"><p className="eyebrow eyebrow--cyan">ADULT PRICING</p><h2>ADULT<br /><em>PRICING.</em></h2></div><div className="pricing-switch" role="group" aria-label="Pricing tier"><button onClick={() => setTier("guest")} className={tier === "guest" ? "is-selected" : ""}>DROP-IN (GUEST)</button><button onClick={() => setTier("member")} className={tier === "member" ? "is-selected" : ""}>SOL CLUB ($239/MO)</button></div><div className="pricing-card"> <div className="pricing-card__head"><span>{tier === "guest" ? "DROP-IN (GUEST)" : "SOL CLUB ($239/MO)"}</span><strong>{tier === "guest" ? "DROP-IN (GUEST)" : "SOL CLUB ($239/MO)"}</strong></div>{comparisons.map((row) => <div className="price-row" key={row.label}><span>{row.label}{row.tip && <Tooltip><TooltipTrigger asChild><button className="info-button" aria-label={`More information about ${row.label}`}><Info size={14} /></button></TooltipTrigger><TooltipContent>{row.tip}</TooltipContent></Tooltip>}</span><b>{tier === "guest" ? row.guest : row.member}</b></div>)}<div className="pricing-card__note"><Tooltip><TooltipTrigger asChild><button>MEMBERSHIP NOTES <Info size={15} /></button></TooltipTrigger><TooltipContent className="membership-tooltip">7-day booking window. 1 guest pass every month. Racket rental on us.</TooltipContent></Tooltip></div><MagneticButton href="#" onClick={() => setCheckoutOpen(true)} className="button--mango">BOOK A SESSION</MagneticButton></div></section>
