...
export default function OurStory() {
  ...
  return (
    <>
      ...
      <AnimatePresence>{drawerContent && <motion.aside className="story-drawer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} aria-modal="true" role="dialog"><motion.div className="story-drawer__panel" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 280, damping: 30 }}><button onClick={() => setDrawer(null)} aria-label="Close story drawer"><X size={27} /></button><span className="eyebrow">SOL SQUASH</span><h2>{drawerContent.title}</h2><p>{drawerContent.body}</p><div className="story-drawer__mural-mark" aria-hidden="true" /></motion.div></motion.aside>}</AnimatePresence>
    </>
  );
}
